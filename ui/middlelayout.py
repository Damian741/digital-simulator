...
from .util import ColumnName, UpdateRelativeRectMixin, UpdateFloatRectMixin, GridUtil
...
class MiddleLayout(BoxLayout):
    def __init__(self, bottom_layout, **kwargs):
        ...
        self.right_connectors = ConnectorBar()
...
class ConnectorBar(GridRelativeLayoutCord, UpdateRelativeRectMixin):
    def __init__(self, bottom_layout=None, **kw):
        super().__init__(**kw)
        with self.canvas:
            Color(0.7, 0.7, 0.7, 1)
        ...
        if bottom_layout:
            ...
            self.label = ColumnName(text="(x,y)", name="ConnectorBar", bcolor=(1, 0, 0, 1))
        ...
        if self.collide_point(*touch.pos):
            ...
            if touch.button == "left":  # left mouse button
                if not self.connector:
                    self.connector = Connector(center=[0, y],  width=self.width)
...
class Connector(Layout):
    def __init__(self, **kwargs):
        ...
        with self.canvas:
            Color(0.2, 0.2, 0.2, 1)
            self.rectangle = Rectangle(pos=[self.width/2, self.y], size=[self.width/2, self.height])
            Color(1, 0.2, 0.2, 1)
            self.line = Line(points=[self.width/2, self.y + self.height/2, self.width, self.y + self.height/2], width=5)
        self.bind(pos=self.update_canvas)

    def update_canvas(self, widget, pos):
        self.rectangle.pos = [self.width/2, pos[1]]
        self.line.points = [self.width/2, pos[1] + self.height/2, self.width, pos[1] + self.height/2]
...
class Board(GridRelativeLayoutCord):
    def __init__(self, bottom_layout, **kwargs):
        ...
        Window.bind(mouse_pos=self.mouse_pos)
        self.padding_y = 5
        self.label = ColumnName(text="(x, y)", name="GlobalCord", bcolor=(0, 1, 0, .5))
        self.local_label = ColumnName(text="(x, y)", name="LocalBoardCord", bcolor=(0, 0, 1, .5))
